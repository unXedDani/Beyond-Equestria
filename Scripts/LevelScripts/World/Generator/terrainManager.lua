	...
	if chunkx ~= lastChunkx or chunky ~= lastChunky then
		for x=-1, 1 do
			...
			if math.abs(chunkx - (lastChunkx+x)) >= 2 then
				removeChunk(lastChunkx+x, lastChunky)
			end
		end
		for x=-1, 1 do
			for y=-1, 1 do
...
function updateChunksa(posx, posy, chunksize, terrainScale)
	local chunkx = math.floor(posx/chunksize)
	local chunky = math.floor(posy/chunksize)
	local half = (chunksize/2)
	local quarter = (chunksize/4)
...
function createChunka(chunkx, chunky, chunksize, terrainScale)
	...
	if heightFactor < 2 then
		...
		MainScene:setMetaData("CHUNKTYPE_"..chunkx.."_"..chunky, 1)
	else
		MainScene:setMetaData("CHUNKTYPE_"..chunkx.."_"..chunky, 0)
	end
	return terrain
end
